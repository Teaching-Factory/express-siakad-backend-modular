...
describe("getHasilPenilaianDosenByDosenIdAndSemesterId", () => {
  ...
  it("should return 200 and hasil penilaian dosen when successful", async () => {
    ...
    const mockDosen = { id: 2, nama_dosen: "Dosen Test" };
    const mockSemester = { id: 1, nama_semester: "Semester Test", TahunAjaran: { nama_tahun: "2024" } };
    ...
    jest.spyOn(Dosen, "findByPk").mockResolvedValue(mockDosen);
    jest.spyOn(Semester, "findByPk").mockResolvedValue(mockSemester);
    ...
    expect(Dosen.findByPk).toHaveBeenCalledWith(2);
    expect(Semester.findByPk).toHaveBeenCalledWith(1, { include: [{ model: TahunAjaran }] });
