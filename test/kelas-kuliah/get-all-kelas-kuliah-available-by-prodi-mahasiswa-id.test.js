...
const { getAllKelasKuliahAvailableByProdiMahasiswaId } = require("../../src/modules/kelas-kuliah/controller");
const { Mahasiswa, KelasKuliah, PesertaKelasKuliah, Prodi, Semester, MataKuliah, Dosen, SettingGlobalSemester } = require("../../models");
...
describe("getAllKelasKuliahAvailableByProdiMahasiswaId", () => {
  ...
  it("should return 404 if no kelas kuliah found", async () => {
    const mockMahasiswa = { id_prodi: 1 };
    const mockSettingGlobalSemester = { id_semester_krs: 1 };
    Mahasiswa.findByPk.mockResolvedValue(mockMahasiswa);
    ...
    expect(KelasKuliah.findAll).toHaveBeenCalledWith({
      where: { id_prodi: mockMahasiswa.id_prodi, id_semester: mockSettingGlobalSemester.id_semester_krs },
  ...
  it("should return available kelas kuliah if found", async () => {
    const mockMahasiswa = { id_prodi: 1 };
    const mockSettingGlobalSemester = { id_semester: 1 };
    ...
    expect(KelasKuliah.findAll).toHaveBeenCalledWith({
      where: { id_prodi: mockMahasiswa.id_prodi, id_semester: mockSettingGlobalSemester.id_semester },
      include: [{ model: Prodi }, { model: Semester }, { model: MataKuliah }, { model: Dosen }]
    });
